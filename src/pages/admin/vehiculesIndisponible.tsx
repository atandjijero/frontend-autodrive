...
export default function VehiculesIndisponible() {
  ...
  useEffect(() => {
    getVehicles()
      .then((res) => {
        ...
        // Filtrer les véhicules indisponibles
        const indisponibles = res.data.filter((v: Vehicle) => !v.disponible);
        setVehicules(indisponibles);
      })
      .catch((err) => {
  ...
  if (vehicules.length === 0) return <p>Aucun véhicule indisponible trouvé.</p>;

  return (
    <>
      <h1 className="text-2xl font-bold mb-4">Véhicules Indisponibles</h1>
